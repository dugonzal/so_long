...
/*   Updated: 2023/01/25 10:54:12 by dugonzal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// el objetivo es recolectar los obetos del mapa
...
int	main(int ac, char **av)
{
	...
	map (&game, av[1]);
	game.mlx = mlx_init();
	game.mlx_win = mlx_new_window(game.mlx, game.map.width * 31, game.map.height * 32, "so_long");
	events (&game);
	map_images (&game);
	create_map (&game);
	mlx_loop(game.mlx);
